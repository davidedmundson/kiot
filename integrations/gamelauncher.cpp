...
/**
    ...
    void ensureConfig()
    {
        ...
        for (auto it = m_games.constBegin(); it != m_games.constEnd(); ++it) {
            ...
            if (!settings.hasKey(configKey)) {
                ...
                configChanged = true;
                qCDebug(gl) << "Added new game to config:" << configKey;
            }
        }
    ...
    bool isSteamInstalled()
    {
        ...
        if (!args.isEmpty()) {
            ...
            if (PlatformHelper::isFlatpak()) {
                KSandbox::ProcessContext ctx = PlatformHelper::makeHostContext(process);
    ...
    QMap<QString, GameData> getHeroicGames()
    {
        ...
        QString flatpakPath = QDir::homePath() + "/.var/app/com.heroicgameslauncher.hgl/config/heroic/";

        if (QDir(nativePath).exists()) {
            heroic_data_path = "/.config/heroic/";
        } else if (QDir(flatpakPath).exists()) {
            heroic_data_path = "/.var/app/com.heroicgameslauncher.hgl/config/heroic/";
        } else {
