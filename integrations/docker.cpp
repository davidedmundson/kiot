...
public:
        ...
        : QObject(parent)
    {
        if (!ensureConfigDefaults()) {
            qCDebug(docker) << "No active container images to monitor";
            qCDebug(docker) << "No active container images to monitor";
            return;
        }
...
private:
     ...
     *
     * @details Starts event listener for Docker container events
     *          sets up a separate thread that listens to Docker socket
     *          for real-time container status updates
     *          This allows automatic UI updates when containers start/stop
     */
...
/**
 * Checks if current user is in the docker group
 *
 * @return bool True if in docker group, false otherwise
 * @brief Verifies that the user is in the docker group for permission
 */
...
void setupDockerSwitch()
{
    if (!isUserInDockerGroup()) {
        ...
        qCWarning(docker) << "Fix this by running: sudo usermod -aG docker $USER and log out/in again.";
        return;
    }

    if (!isDockerAvailable()) {
        qCWarning(docker) << "Docker socket not available. Docker integration will not be started."; //
        return;
    }
